...
export class SupportDialogComponent implements OnInit {
  ...
  constructor(  private dialogRef: MatDialogRef<SupportDialogComponent>,
    ...
    {
      this.isError = true;
      this.errorMsg =  "Для выбранного заказа нет сценария модификации";
    }else if(data.error) {
      this.isError = true;
    ...
    else if(data.scenario === undefined || data.scenario.length == 0)
    {
      ...
      console.log(data.order.scenario);
      this.orderedSteps = data.order.scenario.modificationScenarios;
      this.modsNum = this.orderedSteps.length;
    }
  ...
  prepareData(){
    ...
    this.steps = this.data.scenario.scenarios.split('\r\n');
